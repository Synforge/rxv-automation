...
MenuControl = '<Cursor_Control><Menu_Control>{menu}</Menu_Control></Cursor_Control>'
MenuCursorControl = '<Cursor_Control><Cursor>{action}</Cursor></Cursor_Control>'
...
class RXV(object):
    ...
    def _zone_request(self, command, request_text, zone="Main_Zone"):
        if zone == None:
            payload = request_text
        else:
    ...
    @property
    def config(self):
        response = self._request('GET', ConfigGet.format(src_name='System'), main_zone=False)
    ...
    def zone_power(self, state, zone="Main_Zone"):
        assert state in [True, False]
        new_state = "On" if state else "Standby"
        request_text = PowerControl.format(state=new_state)
        response = self._zone_request('PUT', request_text, zone=zone)
        return response
    ...
    def zone_input(self, input_name, zone="Main_Zone"):
        assert input_name in self.zone_inputs(zone=zone)
        request_text = Input.format(input_name=input_name)
        self._zone_request('PUT', request_text, zone=zone)

    def zone_inputs(self, zone="Main_Zone"):
        if not self._inputs_cache:
            request_text = InputSelItem.format(input_name=GetParam)
            res = self._zone_request('GET', request_text, zone=zone)
    ...
    def menu_open(self, menu):
        request_text = MenuControl.format(menu=menu)
        return self._request('PUT', request_text)

    def menu_cursor(self, action):
        request_text = MenuCursorControl.format(action=action)
        return self._request('PUT', request_text)
    ...
    def zone_volume(self, zone='Main_Zone'):
        request_text = VolumeLevel.format(value=GetParam)
        response = self._zone_request('GET', request_text, zone=zone)
        vol = response.find('{}/Volume/Lvl/Val'.format(zone)).text
        return float(vol) / 10.0
    ...
    def set_zone_volume(self, value, zone='Main_Zone'):
        value = str(int(value * 10))
        exp = 1
        unit = 'dB'
